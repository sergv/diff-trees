...
instance Pretty SexpAtom where
...
sampleSexp :: Sexp
sampleSexp =
  Fix (List [fooPair, fooPair])
  where
    fooSym = Fix (Atom (Symbol "foo"))
    fooPair = Fix (List [fooSym, fooSym])

data SexpSignatureF e =
    SignatureAtom SexpAtom
  | SignatureListCons e e
  | SignatureListNil
  | SignatureVectorCons e e
  | SignatureVectorNil
  deriving (Show, Eq, Ord, Functor, Foldable, Traversable)

type SexpSignature = Fix SexpSignatureF

type SexpTree = Tree (SexpSignatureF Wildcard)

mkSexpTree :: Sexp -> SexpTree
mkSexpTree = cata alg . futu expandComposites
  where
    alg :: SexpSignatureF SexpTree -> SexpTree
    alg x = Fix $ Node (Wildcard <$ x) () $ toList x
    expandComposites :: Sexp -> Free SexpSignatureF Sexp
    expandComposites (Fix (Atom a))  = Free $ SignatureAtom a
    expandComposites (Fix (List xs)) =
      foldr (\x acc -> Free $ SignatureListCons (Pure x) acc) (Free SignatureListNil) xs
    expandComposites (Fix (Vector xs)) =
      foldr (\x acc -> Free $ SignatureVectorCons (Pure x) acc) (Free SignatureVectorNil) xs

diffSexp :: Sexp -> Sexp -> TreeDiff (SexpF Wildcard)
diffSexp x y = diffTrees [mkSexpTree x] [mkSexpTree y]
...
test x y = do
  ...
  TL.putStrLn $ display $ "diff" PP.<$> PP.indent 2 (PP.text $ TL.pack $ show diff)
